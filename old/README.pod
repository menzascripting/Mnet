...
Many unix management tools do not work on specialized network devices. 
For network automation scripts the Mnet perl modules are a more
complete solution than the low level Expect and Net::Telnet
libraries. Mnet facilitates batch processing, advanced logging,
reporting, and repeatable testing.

The Mnet perl modules are far more flexible and less cumbersome to
use than larger enterprise management systems for automation
scripts - getting out of the way and allowing the focus to be on
developing and testing the engineering logic in a script.
...
including autodetection of ssh and telnet, flexible authentication,
bastion host connections, and reliable command prompt handling.
...
- Specialized handling of cisco ios devices, including advanced parsing
and modification of configuration stanzas, scheduled reloads, and more.

- Framework for regression testing of automation scripts, handling
multiple scenarios, integrated with session record/replay capabilities.
